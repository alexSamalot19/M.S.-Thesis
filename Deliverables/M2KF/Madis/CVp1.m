...
Obs=dlmread(fullfile(folderin,['Obsar',num2str(filestorm),'.txt']));
WRFar=dlmread(fullfile(folderin,['WRFar',num2str(filestorm),'.txt']));
KFarr=dlmread(fullfile(folderin,['KFarr',num2str(filestorm),'.txt']));
Loarr=dlmread(fullfile(folderin,['Loarr',num2str(filestorm),'.txt']));
...
%Plotting
 ...
 meanBiasWRF = mean(DWRFMV);
   ...
   dlmwrite('M3MadK2O.txt',latFFArray);
   dlmwrite('M3MadK2K.txt',KALMV);
   dlmwrite('M3MadK2W.txt',WRFMV);
   
   dlmwrite('M3MadK2Kmb.txt',DKalMV);
   dlmwrite('M3MadK2Krmse.txt',DsqKalMV);
   dlmwrite('M3MadK2Wmb.txt',DWRFMV);
   dlmwrite('M3MadK2Wrmse.txt',DsqWRFMV);
...
%    fd=TimeCell{1}
 %   firstDay=cell2mat(fd)
    ...
    % title(titlestring);
    line1 ='WRF & Kalman Filter (2D) Wind Speed Prediction Error 10% Storm History  Madis Stations';
...
%    fd=TimeCell{1}
 %   firstDay=cell2mat(fd)
    % titlestring=['y=',num2str(P(1)),'x+',num2str(P(2)),' R^2=',num2str(R(1,2))];
    % title(titlestring);
    line1 ='Kalman Filter (2D) Wind Speed Prediction Error 10% Storm History Madis Stations';
    %['Station ID:  ',(STelim{j,1}),' ','Storm:  ', firstDay(1:8)]
%     line2 =['UK RMSE (m/s)=',num2str(rmse)];
%     line3=['UK Mean Bias (m/s)=',num2str(meanBias)];
%     line4=['UK R^2=',num2str(R(1,2))];
...
%    fd=TimeCell{1}
 %   firstDay=cell2mat(fd)
    % titlestring=['y=',num2str(P(1)),'x+',num2str(P(2)),' R^2=',num2str(R(1,2))];
    % title(titlestring);
    line1 ='WRF Prediction Error 10% Storm History Madis Stations';
    %['Station ID:  ',(STelim{j,1}),' ','Storm:  ', firstDay(1:8)]
%     line2 =['UK RMSE (m/s)=',num2str(rmse)];
%     line3=['UK Mean Bias (m/s)=',num2str(meanBias)];
%     line4=['UK R^2=',num2str(R(1,2))];
