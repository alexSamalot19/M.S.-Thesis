...
FiMetIn = ['20031214';'20040701';'20040820';'20040908'; ...
...
RefLis=1;
    ...
    fclose(fileID);
totaltime=0;
Kalfirst=zeros(48,(RefLis-1))
%You left off Here Look up!!!!!!%%%%%%%%%%%%%%%%%fix MadCR

for notu=1:numvals(1)
   clearvars -except notu FiMetIn RefLis Irene totaltime Kalfirst
   
stormbeg  = (FiMetIn(notu,:));


    stormname=[stormbeg,'F24.txt']
    kalOut=fullfile('Meth3Met','KalinMet72.txt');
% %    
%numWRF= dlmread(fullfile(AdjFo,'WRF24.txt'));
%MadName=[filebeg,'ff.txt'];
 numIrene = dlmread(fullfile('wfMFFST',stormname));
 % %
...
totaltime=totaltime+1;
    ...
    for tickstats=1:sizeomit;
       stats=heads(tickstats)
        %for tt=totaltime:((totaltime-1)+iend)
           for inner=1:iend
               tt=(totaltime-1)+inner
               if variable(inner,stats)==0;
        error('heads has 0')
        end
               
           Kalfirst(tt,stats)=variable(inner,stats);
           %end
        end
    end
