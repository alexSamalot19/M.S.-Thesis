...
%%Initial time read
Timesize=dlmread(fullfile('GClistM32.txt'));
copyfile(fullfile('GClistM32.txt'), ...
    fullfile('F:','Meth34nBase',Dim,'GClistM32.txt'));
...
%make these proper fopen and close
    ...
    dlmwrite(fullfile('F:','Meth34nBase',Dim,file1), yv);
    
    file2 = [fb2, numbernew, ender];
    dlmwrite(fullfile('F:','Meth34nBase',Dim,file2), x_matrix);
    
    file3 = [fb3, numbernew, ender];
    dlmwrite(fullfile('F:','Meth34nBase',Dim,file3), p_matrix);
    
     file4 = [fb4, numbernew, ender];
    dlmwrite(fullfile('F:','Meth34nBase',Dim,file4), x);
    
     file5 = [fb5, numbernew, ender];
    dlmwrite(fullfile('F:','Meth34nBase',Dim,file5), p_matrix);
    
%     dlmwrite(fullfile(Dim,file6),modtom);    
%     
    
for TFold=1:100%4872
   TF=['TF',num2str(TFold)];
   mkdir(TF)
   ...
   R=1;  
    ...
    dlmwrite(fullfile('F:','Meth34nBase',Dim,TF,KGname), KG);
    dlmwrite(fullfile('F:','Meth34nBase',Dim,TF,TOTx), x);
    dlmwrite(fullfile('F:','Meth34nBase',Dim,TF,TOTp), P);
    dlmwrite(fullfile('F:','Meth34nBase',Dim,TF,TOTq), Q);
