...
fclose(fileID);
WMeoutSN=['BiasMetOut',STstring]

for notu=RN
    disp(notu)
    clearvars -except notu FiMetIn RefLis Irene WMeoutSN
    stormbeg  = (FiMetIn(notu,:));
    
    
    stormname='F24.txt'
    numWRF= dlmread(fullfile(WMeoutSN,'WRF24.txt'));
    %MadName=[filebeg,'ff.txt'];
    numIrene = dlmread(fullfile(WMeoutSN,stormname));
    % %
...
howmany=0;
  ...
  omitfile=[stormbeg,'om.txt']
          dlmwrite(fullfile(WMeoutSN,omitfile),heads)
...
CoutName=[filebeg,'HCn0.txt'];
RoutName=[filebeg,'HRn0.txt'];
SToutName=[filebeg,'HSTn0.txt'];
FFoutName=[filebeg,'HFFn0.txt'];


ST24ID = fopen(fullfile(WMeoutSN,SToutName), 'w');
C24ID = fopen(fullfile(WMeoutSN,CoutName), 'w');
R24ID = fopen(fullfile(WMeoutSN,RoutName),'w');
FF24ID=fopen(fullfile(WMeoutSN,FFoutName),'w');
  
    sizeomit=size(heads)
    for tickstats=1:sizeomit;
       stats=heads(tickstats)
        ...
        error('heads has 0')
            ...
            disp(notu)
            disp(filebeg)
            HBias=numWRF(inner,stats)-variable(inner,stats);
            fprintf(ST24ID, '%s\n', cell2mat(STn0));
            fprintf(C24ID, '%s\n', cell2mat(Cn0));
            fprintf(R24ID, '%s\n', cell2mat(Rn0));
            fprintf(FF24ID, '%f\n', HBias);
...
filebeg  = cell2mat(TimeCell{inner,1});
TaiCoutName=[filebeg,'TCn0.txt'];
TaiRoutName=[filebeg,'TRn0.txt'];
TaiSToutName=[filebeg,'TSTn0.txt'];
TaiFFoutName=[filebeg,'TFFn0.txt'];


ST24ID = fopen(fullfile(WMeoutSN,TaiSToutName), 'w');
C24ID = fopen(fullfile(WMeoutSN,TaiCoutName), 'w');
R24ID = fopen(fullfile(WMeoutSN,TaiRoutName),'w');
FF24ID=fopen(fullfile(WMeoutSN,TaiFFoutName),'w');
  
    sizetai=size(tails);
    for ticktai=1:sizetai;
       stattai=tails(ticktai);
        if variable(inner,stattai)>0;
            
            howmany=howmany+1;
            cel=statname{inner,stattai};
            Cn0=cel(4);
            Rn0=cel(5);
            STn0=cel(1);
            fprintf('writing')
            disp(notu)
            disp(filebeg)
            TBias=numWRF(inner,stats)-variable(inner,stats);
            fprintf(ST24ID, '%s\n', cell2mat(STn0));
            fprintf(C24ID, '%s\n', cell2mat(Cn0));
            fprintf(R24ID, '%s\n', cell2mat(Rn0));
            fprintf(FF24ID, '%f\n', TBias);
