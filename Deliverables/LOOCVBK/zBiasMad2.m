...
WMoutSN=['BiasMadOut',STstring]
for notu=RN
   clearvars -except notu FiMetIn RefLis Irene WMoutSN
stormbeg  = (FiMetIn(notu,:));
    ...
    %[stormbeg,
% %    
numWRF= dlmread(fullfile(WMoutSN,'WRF24.txt'));
%MadName=[filebeg,'ff.txt'];


numIrene = dlmread(fullfile(WMoutSN,stormname));
% %
...
howmany=0;
  ...
  omitfile=[stormbeg,'om.txt']
          dlmwrite(fullfile(WMoutSN,omitfile),heads)
...
ST24ID = fopen(fullfile(WMoutSN,SToutName), 'w');
C24ID = fopen(fullfile(WMoutSN,CoutName), 'w');
R24ID = fopen(fullfile(WMoutSN,RoutName),'w');
FF24ID=fopen(fullfile(WMoutSN,FFoutName),'w');
...
ST24ID = fopen(fullfile(WMoutSN,TaiSToutName), 'w');
C24ID = fopen(fullfile(WMoutSN,TaiCoutName), 'w');
R24ID = fopen(fullfile(WMoutSN,TaiRoutName),'w');
FF24ID=fopen(fullfile(WMoutSN,TaiFFoutName),'w');
