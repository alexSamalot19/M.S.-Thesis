...
for st=1:(Stnum-1)
    ...
    BySTW=['loim1m3',cell2mat(Storm{st,1}),'.mat'];
    ...
    for w=1:(Statnum-1)
        ...
        pkdrivec=exist(fullfile(DriveInfo,DiffFo,['LOIKPFFa',lname,'.csv']));
        ...
        if tinew>tiold
            if pkdrive>0
                ...
                if pkdrivec>0
                    ...
                    Bigtism=Bigtism+1;
                    %KALcompare
                    DRIKALC=dlmread(fullfile('C:', 'Users', 'alex', 'Desktop', ...
            'Storm3Test',BySTatFo,['finMEANkalAR',lname,'.csv']));
                    
                    DRIPKALC=dlmread(fullfile(DriveInfo,DiffFo,['LOIKPFFa',lname,'.csv']));
