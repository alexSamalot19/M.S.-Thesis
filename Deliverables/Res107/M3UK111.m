...
for u = 1:(timnam-2)%ile
   ...
   CmIDtr = fopen(fullfile(AdjFo,(filnCtr)), 'r');
   RmIDtr = fopen(fullfile(AdjFo,(filnRtr)), 'r');
   FFmIDtr = fopen(fullfile(AdjFo,(filnFFtr)), 'r'); 
...
yxn=[filebeg,'loiM3yx.txt'];
msee=[filebeg,'loiM3mse.txt'];
locat=[filebeg,'loiM3CR.txt'];
orig=[filebeg,'loiM3ff.txt'];
