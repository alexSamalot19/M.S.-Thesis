...
% 
    ...
    %%% wrfname=[filebeg2,'WRFp',filendwrite]
    ...
    %TimeID = fopen((Timename), 'w');
     StationID = fopen(fullfile('wfMFFST',(Stationname)), 'w');
    %%% wrfID=fopen(fullfile('WRFpair',wrfname), 'w');
     
    %LatID = fopen((Latname), 'w');
    %LonID = fopen((Lonname), 'w');
    ObsID = fopen(fullfile('wfMFFST',(Obsname)), 'w');
       
        
        %fprintf(TimeID, '%s\n', finTime);
         fprintf(StationID, '%s\n', finStation);
          %%%fprintf(wrfID,'%.001f\n',str2num(cell2mat(Mod{1})));
        %fprintf(LatID, '%.001f \n', finLat);
        %fprintf(LonID, '%.001f \n', finLon);
        fprintf(ObsID, '%.001f \n', finObs);
        prev = filebeg2;
