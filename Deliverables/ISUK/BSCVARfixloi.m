...
STCVID=fopen(fullfile('CEELab20','storm',STCVname),'r');
timnam = 1;
while(~feof(STCVID));
...
%%%%ElimArray
    ...
    ElimFo = fullfile('C:','Users','alex','Desktop','Storm3Test', ...
        BySTatFo,ElimName);
...
%     fullfile(COMP,BySTatFo,ElimName)
%   error('run off of Storm3Test HElims')
...
cd ..
cd (COMP)
        YX =dlmread(fullfile(ZfFo,[filebeg,'loiyx.txt']));
        S = dlmread(fullfile(ZfFo,[filebeg,'loiCR.txt']));
        %disp(lif)
